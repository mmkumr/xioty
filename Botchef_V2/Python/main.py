...
db = firestore.client()
ser = serial.Serial('/dev/pts/5', 250000)
sleep(2)
...
while True:
    if len(mqtt.cmds) != 0:
        ...
        if len(mqtt.cmds) == 1:
            ...
            sendSerialCommand(ser, mqtt.cmds[0].replace('\n', '') + '\r\n')
            mqtt.client.publish(mqtt.MQTT_TOPIC_SEND,
                                "Done", qos=1, retain=True)
        elif len(mqtt.cmds) > 2:
            for i in range(3):
                ser.write(str.encode(cmds[i].replace('\n', '') + '\r\n'))
