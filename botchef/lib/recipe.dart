...
class _RecipeState extends State<Recipe> {
  ...
  void initState() {
    super.initState();
    getmicros();
    widget.recipe['operations'].forEach((k, v) {
      if (v.runtimeType == List) {
        setState(() {
          operations.add(operation(label: k, name: v[0], param: v[1]));
          _params!.add(TextEditingController(text: '1'));
        });
      } else {
        operations.add(operation(label: k, name: v));
      }
    });
  }
